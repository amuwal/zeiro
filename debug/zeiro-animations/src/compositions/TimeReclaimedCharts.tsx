  ...
  return (
    <>
      ...
      <svg width={W} height={H} style={{ marginTop: 14, display: "block" }} aria-hidden="true">
        ...
        {[0.5, 1].map((g) => (
          <line key={g} x1={padL} y1={top + g * (H - top - 18)} x2={W - padR} y2={top + g * (H - top - 18)} stroke={palette.line} strokeWidth={1} strokeDasharray="3 6" />
        ))}
        ...
        {draw > 0.02 && <circle cx={head.x} cy={head.y} r={4.5} fill={palette.accent} stroke={palette.surface} strokeWidth={2} />}
      </svg>
    </>
  );
};
...
const StatTile: React.FC<{ tile: Tile; index: number }> = ({ tile, index }) => {
  ...
  return (
    ...
    >
      <div style={{ fontFamily: fonts.mono, fontSize: 10, color: palette.muted2, letterSpacing: "0.3em", textTransform: "uppercase", marginBottom: 12 }}>
        {tile.en}
      </div>
      <div style={{ display: "flex", alignItems: "baseline", gap: 4 }}>
        <span style={{ fontFamily: fonts.sans, fontSize: 46, fontWeight: 700, color: palette.ink, letterSpacing: "-0.03em", lineHeight: 1, fontVariantNumeric: "tabular-nums" }}>
          {tile.pre}
          {n}
        </span>
        <span style={{ fontFamily: fonts.jp, fontSize: 17, fontWeight: 500, color: palette.accent }}>{tile.suffix}</span>
      </div>
      <div style={{ fontFamily: fonts.jp, fontSize: 15, color: palette.muted, marginTop: 10 }}>{tile.jp}</div>
    </div>
  );
};
