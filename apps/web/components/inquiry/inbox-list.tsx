...
export function InboxList({ items, state }: { items: InboxItemView[]; state: FilterState }) {
  ...
  useLayoutEffect(() => {
    if (!listRef.current || !selectedId || !filteredHasSelected) {
      setPill(null);
      return;
    }
  ...
  }, [selectedId, filteredHasSelected, filtered.length]);
