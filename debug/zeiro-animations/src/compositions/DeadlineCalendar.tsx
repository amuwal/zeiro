...
const RESOLVED_ORDER = Array.from({ length: TOTAL }, (_, i) => i).sort(
  (a, b) => seedAt(a + 7) - seedAt(b + 7),
);
...
export const DeadlineCalendar: React.FC = () => {
  ...
  return (
    <AbsoluteFill>
      ...
      >
        <Counter label="期限超過 → 解消" before={totalOverdue} after={resolvedNum} accent />
        <Counter label="残り 期限超過" before={totalOverdue} after={totalOverdue - resolvedNum} />
        <div
          style={{
            opacity: counterT,
