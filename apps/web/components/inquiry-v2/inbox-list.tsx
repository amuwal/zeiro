    ...
    <div className="inbox-col">
      <div className="inbox-head">
        ...
        <div className="inbox-meta">
          {String(inquiries.length).padStart(2, '0')} / {String(totalCount).padStart(2, '0')}
        </div>
      </div>
      ...
      <div className="inbox-list">
        {inquiries.map((inq, i) => (
          <Link
            key={inq.id}
            href={`/inbox-v2/${inq.id}${searchParams}`}
          ...
          >
            <div className="item-row1">
              <span className="channel-mark" title={inq.channel}>
                {CHANNEL_GLYPH[inq.channel]}
              </span>
              <span className="company">{inq.company}</span>
              {(inq.turnCount ?? 1) > 1 && (
                <span className="turns-mark">{inq.turnCount}</span>
              )}
        ...
        {inquiries.length === 0 && <div className="empty">該当する問い合わせはありません</div>}
      </div>
    </div>
  );
}
